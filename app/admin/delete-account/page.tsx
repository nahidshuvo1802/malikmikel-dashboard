...
import { useGetDeleteAccountQuery, useUpdateDeleteAccountMutation } from "@/store/api/termsApi";

export default function DeleteAccountPage() {
  const { data: apiData, isLoading: isFetching } = useGetDeleteAccountQuery(undefined);
  const [updateDeleteAccount] = useUpdateDeleteAccountMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateDeleteAccount({ description: content }).unwrap();
      toast.success("Delete Account Info updated successfully!");
    } catch (error) {
      toast.error("Failed to update Delete Account Info");
    } finally {
  ...
  return (
    <div className="w-full mx-auto">
      <div className={`${buttonbg} px-6 py-4 rounded-xl mb-6 flex items-center gap-3 shadow-md shadow-blue-200`}>
        ...
        <h1 className="text-white text-xl sm:text-2xl font-bold">Delete Account Info</h1>
      </div>
